...
class TDSRnnCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        rnn_out = rnn_hidden_size * 2 if rnn_bidirectional else rnn_hidden_size
...
class TDSConvTransformerCTCModule(pl.LightningModule):
    ...
    def _step(
        ...
        T_diff = inputs.shape[0] - emissions.shape[0]
        emission_lengths = input_lengths - T_diff
        #subsample_reduction = inputs.shape[0] - (emissions.shape[0] * self.subsample)
        #emission_lengths = (input_lengths - subsample_reduction) // self.subsample
...
class AutoEncoderModule(pl.LightningModule):
    ...
    def _step(self, phase:str, batch : dict[str, torch.Tensor], *args, **kwargs):
      ...
      loss = self.mse_loss(pred, inputs)
